	...
	*/
	public function render()
	{
		...
		foreach($params as $param)
		{
			foreach($param as $_param)
			{
				...
				{
					$html[] = '<td class="label"><span class="editlinktip hasTip" title="'.(empty($_param['description']) ? '' : JText::_($_param['description'] )).'">'. JText::_($_param['label']) .'</span></td>';
	...
	private function _getRadio($data,$value)
	{
		...
		foreach($data->children() as $_data)
		{
			...
			$html[] = '<input title="'.(empty($data['description']) ? '' : JText::_($data['description'] )).'" id="params'.$name.$_data['value'].'" type="radio" name="params['.$name.']" value='.$_data['value'].$selected.' />';
