...
$doc->addScript($url . 'components/com_community/assets/slide_list_friends.js');
...
<?php if( !empty( $data ) ) { ?>
	...
	<ul class="cIndexList forFriendsList cResetList sugerencias">
	<?php foreach( $data as $row ) : ?>
		<?php 
			$displayname = $row->user->getDisplayName();
			$isWaitingApproval = CFriendsHelper::isWaitingApproval($my->id, $row->user->id);
		 ?>
		<?php if(!empty($row->user->id) && !empty($displayname) && ($row->user->_userid != $user->id) && ($row->addFriend) && (!$isWaitingApproval)) : ?>
		<li>
        <?php //var_dump($row);?>
		<div class="cIndex-Box clearfix">
			...
			<div class="cIndex-Content">
				...
				<div class="cIndex-Actions">
					...
					?>
                    	<div>
                        	<a class="no_acepta"><i class="com-icon-report-shade"></i><span>NO</span></a>
                        </div>
                       	<span>¿Quieres conocerlo(a)?</span>
						<div>
							<?php /*if(isset($row->isMyFriend) && $row->isMyFriend==1)*/if(false){ ?>
								<i class="com-icon-info"></i>
								<a href="javascript:void(0)" onclick="console.log( joms.friends.connect('<?php echo $row->user->id;?>') ); "><span><?php echo JText::_('COM_COMMUNITY_PROFILE_PENDING_FRIEND_REQUEST'); ?></span></a>
							<?php } else { ?>
								<?php if(!$isWaitingApproval){?>
									<!--<i class="com-icon-user-plus"></i>-->
									<a href="javascript:void(0)" onclick="joms.friends.connect('<?php echo $row->user->id;?>')" class="boton_add"><i class="com-icon-tick"></i><span><?php echo JText::_('COM_COMMUNITY_PROFILE_ADD_AS_FRIEND'); ?></span></a>
								<?php }else{ ?>
