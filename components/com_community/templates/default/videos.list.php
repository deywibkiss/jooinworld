				...
				<div class="visible-desktop">
					...
					<div class="cMedia-Actions small<?php if( !in_array($video->id, $featuredList) ) { ?> featured<?php } ?> ">
						<div>
							...
							<a title="<?php echo JText::_('COM_COMMUNITY_DELETE') ?>" href="javascript:void(0);" onclick="joms.videos.deleteVideo('<?php echo $video->getId();?>','<?php echo $currentTask;?>');">
								<i class="com-icon-block"></i>
								<?php // echo JText::_('COM_COMMUNITY_DELETE') ?>
							</a>
							<?php } ?>
							<?php if( $isCommunityAdmin && !$groupVideo && $showFeatured && $video->permissions == 0) {
								if( !in_array($video->id, $featuredList) )
								{
							?>
