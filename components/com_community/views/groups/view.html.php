...
class CommunityViewGroups extends CommunityView {
    ...
    public function viewGroup($group) {
        ...
        if (( ($isAdmin || $isSuperAdmin) && $params->get('photopermission') == 1) || ( ($isMember || $isSuperAdmin) && $params->get('photopermission') == 2))
            $creators[] = CUserStatusCreator::getPhotoInstance();
        if (( ($isAdmin || $isSuperAdmin) && $params->get('videopermission') == 1) || ( ($isMember || $isSuperAdmin) && $params->get('videopermission') == 2))
            $creators[] = CUserStatusCreator::getVideoInstance();
        if ( ($allowCreateEvent  || $isSuperAdmin ) && $config->get('group_events') && $config->get('enableevents') && ($config->get('createevents') ))
