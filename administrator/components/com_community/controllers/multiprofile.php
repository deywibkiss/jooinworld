...
class CommunityControllerMultiProfile extends CommunityController
{
	...
	public function save()
	{
		...
		$fields		= $jinput->get('fields' , '', 'NONE') ; //JRequest::getVar( 'fields' , '' );
		$name		= $jinput->get('name' , '', 'STRING') ; //JRequest::getVar( 'name' , '' );
		$tmpParents	= $jinput->get('parents' , '', 'NONE') ; //JRequest::getVar( 'parents' , '' );
		$mainframe	= JFactory::getApplication();
		...
		if(!empty($watermark['tmp_name'])){
			...
			if( isset($watermark['name']) && !empty($watermark['name']) )
			{
				...
				*/				
				...
				{
					$idParam = $id?"&id=".$id:'';
					$mainframe->redirect( 'index.php?option=com_community&view=multiprofile&layout=edit'.$idParam , JText::_( 'COM_COMMUNITY_MULTIPROFILE_WATERMARK_IMAGE_EXCEEDS_SIZE' ) , 'error' );
					exit;
				}
			}
		}
