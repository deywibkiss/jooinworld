...
class CommunityViewUsers extends JViewLegacy
{
	...
	public function display( $tpl = null )
	{
		...
		$search				= $jinput->get('search' , '', 'STRING'); //JRequest::getVar( 'search' , '' );
		...
		$usertype			= $jinput->post->get('usertype' , 'all', 'NONE'); //JRequest::getVar('usertype' , 'all' , 'POST' );
		$multiprofileModel	= $this->getModel( 'Multiprofile' );
		$profileTypes		= $multiprofileModel->getMultiprofiles();
		$profileType		= $jinput->post->get('profiletype' , 'all', 'NONE'); //JRequest::getVar( 'profiletype' , 'all' , 'POST' );
	...
	public function _displayEditLayout( $tpl )
	{
		...
		$userId		= $jinput->request->get('id' , '', 'INT'); //JRequest::getVar( 'id' , '' , 'REQUEST' );
	...
	public function getPublish( &$row , $type , $ajaxTask )
	{
		...
		if($currentV >= '0.30')
		{
			$class = $row->$type == 0 ? 'btn btn-micro disabled jgrid': 'btn btn-micro ';
